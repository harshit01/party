...
"$APP" -batchmode -nographics -partyrole host -partytarget "$PARTICIPANTS" \
       -partyround -partyrounds "$ROUNDS" -partybiasseed "$SEED" -partyseconds $((ROUNDS * 80)) \
...
"$APP" -batchmode -nographics -partyrole host -partytarget 4 \
       -partyround -partyrounds 3 -partybiasseed "$SEED" -partyseconds 240 \
...
if curl -s -m 2 http://127.0.0.1:8790/health >/dev/null 2>&1; then
  ...
  "$APP" -batchmode -nographics -partyrole host -partytarget 4 \
         -partyround -partyrounds 2 -partybiasseed "$SEED" -partyseconds 180 \
